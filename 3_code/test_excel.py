...
class SiameseNetworkTestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img0_tuple = ['./test5/testset/s{}/img1.jpg'.format(SiameseNetworkTestDataset.k), SiameseNetworkTestDataset.k-1]
        img1_tuple = [Config.testing_dir + '/s{}/img{}.jpg'.format(SiameseNetworkTestDataset.i, SiameseNetworkTestDataset.j), SiameseNetworkTestDataset.i - 1]

        SiameseNetworkTestDataset.cnt += 1
        SiameseNetworkTestDataset.i += 1
        print(SiameseNetworkTestDataset.cnt)
        if SiameseNetworkTestDataset.i > 10 :
            SiameseNetworkTestDataset.i = 1
            SiameseNetworkTestDataset.j += 1
            if SiameseNetworkTestDataset.cnt == 90:
...
transform=transforms.Compose([transforms.Resize((128, 64)),transforms.ToTensor()]), # 이미지 크기 100, 100 사이즈로 reshape, Numpy -> tensor(0에서 1사이의 값으로 정규화)
should_invert=False)
...
gx = 0
gy = 0
distance_data = [[] for _ in range(900)]
...
df1.to_excel(excel_writer='./test_result.xlsx')